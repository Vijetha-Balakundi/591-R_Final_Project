...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Sample Information Exploration",
             ...
             sidebarLayout(
               sidebarPanel(
                 fileInput("Sample_file", "Choose a Sample Information CSV File", accept = ".csv", placeholder = "sample information file"),
               ...
               # subtabs for first main tab
                 tabsetPanel(
                   ...
                   tabPanel("Table", p("This tab provides the sortable sample information table."), DT::dataTableOutput("sample_table")),
                   tabPanel("Plots", p("This tab provides a histogram plot for selected characteristic of samples."),
                            sidebarLayout(
                              sidebarPanel(
    ...
    tabPanel("Counts Matrix Exploration",
             ...
             sidebarLayout(
               sidebarPanel(
                 fileInput("Normalized_counts_matrix", "Choose a Normalized Counts Matrix CSV File", accept = ".csv", placeholder = 'normalized counts matrix file'),
               ...
               # subtabs for second main tab
                 tabsetPanel(
                   ...
                   tabPanel("Table or Text",
                            p("This tab provides the statistics based on the input parameters, percentile of variance, and number of samples that are non-zero."),
                   ...
                   tabPanel("Heat map",
                            p("This tab provides a heat map of the normalized counts input matrix"), 
                            plotOutput("heatmap")),
                   tabPanel("pca", 
                            sidebarLayout(
                              sidebarPanel(
                                selectInput(inputId = "comp1", label="Select X-axis", choices = c("PC1", "PC2", "PC3", "PC4", "PC5", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10")),
                                selectInput(inputId = "comp2", label="Select Y-axis", choices = c("PC1", "PC2", "PC3", "PC4", "PC5", "PC5", "PC6", "PC7", "PC8", "PC9", "PC10"), selected = "PC2"),
                                
                              ),
                              mainPanel(p("This tab provides a pca plot of PC1 and PC2 components"),
    ...
    tabPanel("Differential Expression",
             ...
             sidebarLayout(
               sidebarPanel(
                 fileInput("DE_Anaylsis", "Choose a Differential Expression Results CSV File", accept = ".csv",placeholder = 'differential expression analysis file'),
               ...
               # subtabs for third main tab
                 tabsetPanel(
                   type = "tabs",
                   tabPanel("Table",
                            p("This tab provides sortable table of the differential expression analysis results."),
                            DT::dataTableOutput("table",width = "80%")),
                   tabPanel("Volcano Plot", 
                            p("This tab provides a volcano plot based on the input paramters, namely, X-axis and Y-axis"),
                            sidebarLayout(
                              sidebarPanel(
    ...
    tabPanel("Visualization of Individual Gene Expression",
             ...
             sidebarLayout(
               sidebarPanel(
                 ...
                 fileInput(inputId = 'IGE_metaID', label = 'Choose a Sample Information CSV File',accept = ".csv", placeholder = "sample information file"),
                 selectInput("metachoice", choices = c("Sample_diagnosis"="Sample_diagnosis", "Sample_pmi"="Sample_pmi","Sample_Age_of_Death"="Sample_Age_of_Death", "Sample_rin"="Sample_rin", "Sample_mrna_seq"="Sample_mrna_seq"),
                             label = "Select Metadata Category Field", selected = "Sample_diagnosis"),
      ...
      if (meta_cat == "Sample_diagnosis"){
        ...
        if(type_plot == "Bar Plot"){
          plot <- ggplot(plot_tib, aes(meta_value,Gene_Counts)) +
            ...
            labs(title = paste("Gene Expression of", input$gene, "by", input$metachoice),
